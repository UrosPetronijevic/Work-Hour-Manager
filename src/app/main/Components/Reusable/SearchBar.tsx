...
export default function SearchBar({ items }: ProfilesProps) {
  ...
  const handleSearch = (event: any) => {
    ...
    if (term) {
      ...
      setSearchResults(items); // Reset to original list
    }
  };
  return (
    <div className="w-1/2 bg-red-300 py-2 px-4 rounded-[100px] flex justify-between">
      <input
        className="w-7/8 bg-transparent outline-none"
        type="text"
      ...
      <svg
        className="h-6 w-6 text-gray-500 pointer-events-none"
        fill="#231F20" // Set fill color here
        viewBox="0 0 64 64"
